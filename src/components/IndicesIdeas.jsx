...
const IndicesIdeas = () => {
    const articles = Array(6).fill(null).map((_, i) => ({
        id: i,
        title: "Update US500 Trading Strategy for November 2025",
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-16">
                    {articles.map((item) => (
                        <div key={item.id} className="group cursor-pointer">
                            <h3 className="text-white text-xl font-bold mb-6 leading-snug group-hover:text-[#4DA6FF] transition-colors line-clamp-2">
                                {item.title}
                            </h3>
                            ...
                            <div className="mt-8 border-b border-white/10 w-full group-hover:border-[#4DA6FF]/50 transition-colors"></div>
                        </div>
                    ))}
                </div>
            </div>
