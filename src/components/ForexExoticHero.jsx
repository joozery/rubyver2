...
import heroImg from '../assets/EXOTIC/exotic.jpg';
import { Link } from 'react-router-dom';

const ForexExoticHero = () => {
    const categories = [
    ...
    return (
        <section className="relative w-full min-h-[600px] lg:h-[700px] flex items-center overflow-hidden pt-10">
                    ...
                    <div className="flex items-center gap-6 lg:gap-10 border-b border-white/20 pb-4 overflow-x-auto no-scrollbar">
                        {categories.map((cat, index) => (
                            ...
                            >
                                {cat}
                                {cat === 'Forex Exotic' && (
                                    <span className="absolute bottom-[-17px] left-1/2 -translate-x-1/2 w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-b-[6px] border-b-white"></span>
                                )}
                            </Link>
                        ))}
                    </div>
            ...
            <div className="container mx-auto px-6 lg:px-12 relative z-20 flex flex-col-reverse lg:flex-row items-center h-full mt-16">
                {/* Text Content */}
                <div className="w-full lg:w-1/2 text-center lg:text-left z-20 pt-12">
                    <h1 className="text-[48px] lg:text-[72px] font-black uppercase leading-[0.9] text-white mb-6 tracking-tighter drop-shadow-md">
                        TRADE EXOTIC <br />
                        FOREX <span className="text-[#EBCBA5]">WITH RUBY</span>
                    </h1>

                    <p className="text-lg lg:text-xl text-white font-medium mb-10 max-w-lg mx-auto lg:mx-0 leading-relaxed drop-shadow-sm">
                        Test your ideas by trading currency pairs from emerging markets.
                    </p>

                    <button className="bg-[#EBCBA5] text-black px-10 py-4 rounded-md font-bold text-lg hover:bg-[#d9b894] transition-all shadow-lg hover:shadow-xl hover:-translate-y-1">
                        Trade with RUBY
                    </button>
                </div>
            </div>
...
export default ForexExoticHero;
